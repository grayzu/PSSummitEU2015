function Get-TargetResource
{
    ...
    if($AppPool -ne $null)
    {
        $Ensure = "Present"
        $State  = $AppPool.state
        $Enable32BitAppOnWin64 = (get-webconfiguration "/system.applicationHost/applicationPools/add[@name=`'$Name`']/@enable32BitAppOnWin64").value
    }
...
function Set-TargetResource
{
    ...
    else
    {
        ...
        if($AppPool.Enable32BitAppOnWin64 -ne $Enable32BitAppOnWin64)
        {
            set-webconfiguration "/system.applicationHost/applicationPools/add[@name=$sitename]/@enable32BitAppOnWin64" -Value $Enable32BitAppOnWin64
        }
    }
}
...
function Test-TargetResource
{
    ...
    {
        if($WebAppPool.Ensure -eq $Ensure -and $WebAppPool.State -eq $state -and $WebAppPool.Enable32BitAppOnWin64 -eq $Enable32BitAppOnWin64)
        {
            return $true
        }
    }
